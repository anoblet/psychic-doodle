...
export class BaseCollectionModel {
    ...
    async get() {
        await measure({ endpoint: this._endpoint, params: this.params }, async () => {
            const { data, headers } = (await WooCommerce({ version: this._version }).get(this._endpoint, this.params));
