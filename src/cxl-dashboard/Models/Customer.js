...
export class Customer extends BaseItemModel {
    ...
    get currency() {
        return this.orders && this.orders.total > 0 && this.orders.items[this.orders.total - 1].currency;
    }
    ...
    get isPayingCustomerFormatted() {
        return this.is_paying_customer ? `<span style="color: green">Yes</span>` : `No`;
    }
    ...
    get productName() {
        return this._subscriptions && this._subscriptions.total > 0 && this._subscriptions.items[0].product_name;
    }

    get subscriberSince() {
        return this._subscriptions && this._subscriptions.total > 0 && formatDate(this._subscriptions.items[0].date_created);
    }
    ...
    async getMemberships() {
        if (!this._memberships) {
            this._memberships = (await new MembershipCollection({ params: { customer: this.id } }).get());
        }
    ...
    async getOrders() {
        if (!this.orders) {
            this.orders = (await new OrderCollection({ params: { customer: this.id } }).get());
        }
    ...
    async getSubscriptions() {
        if (!this._subscriptions) {
            this._subscriptions = (await new SubscriptionCollection({ params: { customer: this.id } }).get());
        }
    ...
    async save() {
        await measure({ endpoint: this._endpoint }, async () => {
            this._response = await WooCommerce({ version: this._version }).put(this._endpoint, this._updates);
        });
