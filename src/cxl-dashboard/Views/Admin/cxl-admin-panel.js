...
export class CXLAdminPanelElement extends BaseElement {
  ...
  render() {
    return html`
        <vaadin-button @click=${this._createCustomer}>Create Customer</vaadin-button>
    `;
  }
  ...
  async _createCustomer() {
    ...
    try {
      ...
      if (response.status === 201) {
        this._notification({ message: "Customer created!", theme: "success" });
      }
    } catch (e) {
      this._notification({ messeage: "Unable to create customer", theme: "error" });
      console.log(e);
    }
  ...
  async _createOrder({ customer }) {
    ...
    Object.assign(sampleOrder, { billing: customer.billing, customer_id: customer.id, shipping: customer.shipping });

    try {
      response = await WooCommerce().post("orders", sampleOrder);
      if (response.status === 201) {
        this._notification({ message: "Order created!", theme: "success" });
      }
    } catch (e) {
      this._notification({ messeage: "Unable to create order", theme: "error" });
    }
  }
