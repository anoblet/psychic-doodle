...
export class CXLRefundCreateElement extends ViewElement {
  ...
  render() {
    ...
    <div class="grid gap">
      <vaadin-form-item>
        <vaadin-number-field disabled label="Order ID:" value=${this.item?.orderId} required></vaadin-number-field>
      </vaadin-form-item>
      <div id="lineItems" class="full-width grid gap">
        ${this.item?.order?.lineItems.map((item, index) => html`
        <vaadin-text-field disabled label="Product Name" value=${item.name}></vaadin-text-field>
        <vaadin-number-field label="Total" value=${item.total} required>
          <div slot="prefix">$</div>
        </vaadin-number-field>
        <vaadin-number-field @input=${this._update} data-index=${index} label="Refund Amount" value="0" required>
          <div slot="prefix">$</div>
        </vaadin-number-field>
        `)}
        <vaadin-number-field label="Refund Total" value=${this.item?.amount} required>
          <div slot="prefix">$</div>
        </vaadin-number-field>
      </div>
      <vaadin-text-area @input=${this._setText} class="full-width" label="Reason" name="reason"></vaadin-text-area>
      <hr>
  ...
  async updated(changedProperties) {
    ...
    if (changedProperties.has("item") && (changedProperties.get("item") || {}).orderId !== this.item?.orderId) {
  ...
  async _save() {
    try {
      ...
      notification({ message: html`<span style="flex: 1; text-align: center;">${this._itemName} created!</span>`, theme: "success" }, this.shadowRoot);
      Router.go(`refund/view/${this.item.orderId}/${response.data.id}`);
    } catch (error) {
      notification({ message: error.response.data.message, theme: "error" }, this.shadowRoot);
    }
  }
